...
def causal_conv1d_update(
    ...
    if retrieve_next_token is None and retrieve_parent_token is None and seqlen > 1:
        k = width - 1
        if conv_state_indices is not None:
            cur_conv_state = conv_state[conv_state_indices]
        else:
            cur_conv_state = conv_state[:batch]
        if cur_conv_state.shape[-1] > k:
            cur_conv_state = cur_conv_state[:, :, -k:]
            
        full_x = torch.cat([cur_conv_state, x], dim=-1)
        w = weight.unsqueeze(1)
        out = torch.nn.functional.conv1d(full_x, w, bias=bias, groups=dim)
        if activation in ("silu", "swish"):
            out = torch.nn.functional.silu(out)
            
        if intermediate_conv_window is not None:
            windows = full_x.unfold(dimension=-1, size=k, step=1)[:, :, 1:seqlen+1]
            perm = windows.permute(0, 2, 1, 3)
            if intermediate_state_indices is not None:
                intermediate_conv_window[intermediate_state_indices, :seqlen].copy_(perm)
            else:
                intermediate_conv_window[:batch, :seqlen].copy_(perm)
                
        if conv_state_indices is not None:
            conv_state[conv_state_indices, :, -k:] = full_x[:, :, -k:]
        else:
            conv_state[:batch, :, -k:] = full_x[:, :, -k:]
            
        return out if not unsqueeze else out.squeeze(-1)
    # conv_state: (..., dim, state_len), where state_len >= width - 1
